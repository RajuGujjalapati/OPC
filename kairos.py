...
class Kairos:
    ...
    def update_or_set_metrics_aggregators(self, value):
        if "aggregators" in self.query["metrics"][0]:
            self.query["metrics"][0]["aggregators"] = self.query["metrics"][0]["aggregators"] + value
        else:
            self.set_metrics_aggregators(value)
    ...
    # value must be string
    def set_update_query_name(self, value):
        self.update_query["name"] = value

    # method to set update query datapoints
    # value must be array of arrays
    def set_update_query_datapoints(self, value):
        self.update_query["datapoints"].append(value)

    # method to set update query tags
    # value must be object
    def set_update_query_tags(self, value):
        self.update_query["tags"] = value

    # method to delete metrics aggregator
    def delete_metrics_aggregators(self):
        if "aggregators" in self.query["metrics"][0]:
            del self.query["metrics"][0]["aggregators"]
